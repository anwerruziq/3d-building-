...
// Kept for type compatibility with existing imports (no longer used for input)
...
function City({ onReady, onMeshInfo }: { onReady: (g: THREE.Group) => void; onMeshInfo: (info: string) => void }) {
  // Enable DRACO compression for the GLTF loader (significant size reduction)
  const draco = useMemo(() => {
    const loader = new DRACOLoader();
    loader.setDecoderPath("/draco/" ); // place draco decoder files in public/draco
    return loader;
  }, []);
    ...
    s.traverse((obj) => {
      ...
      if (m.isMesh) {
        ...
        const apply = (mm: THREE.MeshStandardMaterial) => {
          ...
          // Disable expensive shadow casting on mobile devices
          mm.castShadow = false;
    ...
    <Billboard position={[actualP.x, actualP.y + 11, actualP.z]} follow>
      <group ref={ref} scale={0.001}>
        <mesh>
          ...
          <meshBasicMaterial color="#050818" transparent opacity={0.95} />
        </mesh>
        ...
        <Text font="/fonts/Cairo-Bold.ttf" position={[12, 4.5, 0.05]} fontSize={0.7} color={s.color} anchorX="right" anchorY="top" letterSpacing={0.05} fontWeight={700}>
          {s.eyebrow}
        </Text>
        <Text font="/fonts/Cairo-Bold.ttf" position={[12, 2.5, 0.05]} fontSize={2} color="#ffffff" anchorX="right" anchorY="top" maxWidth={16} lineHeight={1.2} fontWeight={900}>
          {s.title}
        </Text>
        <Text font="/fonts/Cairo-Regular.ttf" position={[12, -2.5, 0.05]} fontSize={0.8} color="#ffffff" anchorX="right" anchorY="top" maxWidth={16} lineHeight={1.6}>
          {s.body}
        </Text>
        <Text font="/fonts/Cairo-Bold.ttf" position={[12, -5.2, 0.05]} fontSize={0.6} color={s.color} anchorX="right" anchorY="bottom" letterSpacing={0.1}>
...
function Controller({
  ...
  useFrame((_, delta) => {
    ...
    smoothT.current = THREE.MathUtils.damp(smoothT.current, rawT, 1.5, clampedDelta);
    const t = smoothT.current;
    const pos = curve.getPointAt(t);
    const tangent = curve.getTangent(t).normalize();
    const targetPos = pos.clone();
    targetPos.y += HEIGHT;
    smoothPos.x = THREE.MathUtils.damp(smoothPos.x, targetPos.x, 2, clampedDelta);
    smoothPos.y = THREE.MathUtils.damp(smoothPos.y, targetPos.y, 2, clampedDelta);
    smoothPos.z = THREE.MathUtils.damp(smoothPos.z, targetPos.z, 2, clampedDelta);
    camera.position.copy(smoothPos);
    const lookAhead = pos.clone().add(tangent.clone().multiplyScalar(25));
    lookAhead.y = pos.y + HEIGHT;
    smoothLookAt.x = THREE.MathUtils.damp(smoothLookAt.x, lookAhead.x, 1.5, clampedDelta);
    smoothLookAt.y = THREE.MathUtils.damp(smoothLookAt.y, lookAhead.y, 1.5, clampedDelta);
    smoothLookAt.z = THREE.MathUtils.damp(smoothLookAt.z, lookAhead.z, 1.5, clampedDelta);
    camera.lookAt(smoothLookAt);
  });
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      >
        ...
        <Controller curve={curve} scrollRef={scrollRef} />
...
function City({ onReady, onMeshInfo }: { onReady: (g: THREE.Group) => void; onMeshInfo: (info: string) => void }) {
  const { scene } = useGLTF("/models/future_city.glb");
  const ref = useRef<THREE.Group>(null);
  ...
  const { scale, offset } = useMemo(() => {
    ...
    // Collect scaled coordinates for street meshes
    const streetDetails: string[] = [];
    cloned.traverse((obj) => {
      if (obj.type === "Mesh" && /road|street|path|ground|floor|asphalt|highway/i.test(obj.name)) {
        const m = obj as THREE.Mesh;
        ...
        // Apply scaling and offset to get final world coordinates
        const worldCenter = mCenter.clone().multiplyScalar(s).add(off);
        const worldSize = mSize.clone().multiplyScalar(s);
        streetDetails.push(`${obj.name}:\n  center=[${worldCenter.x.toFixed(1)}, ${worldCenter.y.toFixed(1)}, ${worldCenter.z.toFixed(1)}]\n  size=[${worldSize.x.toFixed(1)}, ${worldSize.y.toFixed(1)}, ${worldSize.z.toFixed(1)}]`);
      }
  ...
  useEffect(() => {
    if (ref.current) {
      onReady(ref.current);
      const details: string[] = [];
      // Force update matrices to ensure setFromObject retrieves correct scaled world coordinates
      ref.current.updateMatrixWorld(true);
      ref.current.traverse((obj) => {
        if (obj.type === "Mesh") {
          const m = obj as THREE.Mesh;
...
function AnimatedBillboard({ s, idx, p }: { s: any, idx: number, p: THREE.Vector3 }) {
  ...
  useFrame((_, delta) => {
    ...
    // When dist is < 160, start showing. Fully shown at < 90.
    const maxDist = 160;
    const minDist = 90;
...
function Controller({
  ...
  const { camera } = useThree();
  const smoothT = useRef(0);           // smoothed scroll parameter
  const smoothPos = useMemo(() => new THREE.Vector3(), []);
  const smoothLookAt = useMemo(() => new THREE.Vector3(), []);
  const HEIGHT = 8.0;                  // constant camera height — no vertical bobbing
  ...
  useFrame((_, delta) => {
    const clampedDelta = Math.min(delta, 0.05);          // cap large frame spikes

    // 1. Smooth the scroll value itself (prevents micro-jitter from scroll events)
    const rawT = THREE.MathUtils.clamp(scrollRef.current, 0, 0.99);
    smoothT.current = THREE.MathUtils.damp(smoothT.current, rawT, 1.5, clampedDelta);

    // 2. Sample position & tangent from the smoothed parameter
    const t = smoothT.current;
    const pos = curve.getPointAt(t);
    const tangent = curve.getTangent(t).normalize();

    // 3. Target position — fixed height, no sudden jumps
    const targetPos = pos.clone();
    targetPos.y += HEIGHT;
    smoothPos.x = THREE.MathUtils.damp(smoothPos.x, targetPos.x, 2, clampedDelta);
    smoothPos.y = THREE.MathUtils.damp(smoothPos.y, targetPos.y, 2, clampedDelta);
    ...
    // 4. Look-ahead — far enough to keep rotation stable through turns
    const lookAhead = pos.clone().add(tangent.clone().multiplyScalar(25));
    lookAhead.y = pos.y + HEIGHT;
    smoothLookAt.x = THREE.MathUtils.damp(smoothLookAt.x, lookAhead.x, 1.5, clampedDelta);
    smoothLookAt.y = THREE.MathUtils.damp(smoothLookAt.y, lookAhead.y, 1.5, clampedDelta);
